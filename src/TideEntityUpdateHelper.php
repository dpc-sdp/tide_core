...
class TideEntityUpdateHelper extends ConfigReverter {
   ...
   *
   * @example
   *  $storage = Drupal::entityTypeManager()->getStorage('field_config')->load('node.landing_page.field_landing_page_component');
   *  $settings = $storage->getSettings();
   *  $positions = $settings['handler_settings']['target_bundles_drag_drop'];
   *  $r = $tide_helper->repositionComponentsRelativeToKey($positions,['basic_text'=>['enabled'=>TRUE]],'accordion','after');
   *  $settings['handler_settings']['target_bundles_drag_drop'] = $r;
