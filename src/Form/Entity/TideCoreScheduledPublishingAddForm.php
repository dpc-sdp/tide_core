...
namespace Drupal\tide_core\Form\Entity;
...
/**
 * Tide Core Scheduled publishing add form.
 */
class TideCoreScheduledPublishingAddForm extends ContentEntityForm {
  ...
  /**
   * Constructs a new TideCoreScheduledPublishingAddForm.
   *
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    $this->messenger()->addMessage($this->t('Scheduled a publishing for @date', [
      '@date' => $this->dateFormatter->format($onDate->getTimestamp(), 'custom', 'd-M-Y - h:i a'),
    ]));
  }
  ...
  protected function actions(array $form, FormStateInterface $form_state): array {
    ...
    $actions['submit'] = [
      '#type' => 'submit',
      '#value' => $this->t('Schedule publishing'),
      '#submit' => ['::submitForm'],
    ];
    return $actions;
  }
      ...
      function (EntityInterface $entityRevision) use ($workflowStates): array {
        ...
        if ($entityRevision instanceof RevisionLogInterface) {
          $option['revision_time']['data']['#plain_text'] = $this->dateFormatter
            ->format($entityRevision->getRevisionCreationTime(), 'custom', 'd-M-Y - h:i a');
