...
namespace Drupal\tide_event\Hook;
...
/**
 * Hook implementations for the tide event module.
 */
class TideEventHooks {
  ...
  public function searchApiIndexItemsAlter(IndexInterface $index, array &$items) {
    foreach ($items as $item_id => $item) {
      ...
      // This should be redundant if we move to consolidating all index alters
      // to a processor.
  ...
  public function webformSubmissionPresave($submission) {
    ...
    if ($submission->getWebform()->get('id') === 'tide_event_submission') {
      ...
      $escape = static fn($value): string => Html::escape((string) ($value ?? ''));
      $location = is_array($submission_data['location'] ?? NULL) ? $submission_data['location'] : [];

      // Add Event Details.
      $event_details_data = [
        'type' => 'event_details',
        'field_paragraph_link' => [
          'uri' => $escape($submission_data['website_url_for_booking'] ?? NULL),
          'title' => t('Book now'),
        ],
        'field_paragraph_date_range' => [
          [
            'value' => date('Y-m-d\TH:i:00', strtotime($escape($submission_data['open_date'] ?? NULL))),
            'end_value' => date('Y-m-d\TH:i:00', strtotime($escape($submission_data['close_date'] ?? NULL))),
          ],
        ],
        'field_paragraph_location' => [
          [
            ...
            'locality' => $escape($location['locality'] ?? NULL),
            'postal_code' => $escape($location['postal_code'] ?? NULL),
            'address_line1' => $escape($location['address_line1'] ?? NULL),
            'address_line2' => $escape($location['address_line2'] ?? NULL),
          ],
        ],
        'field_paragraph_event_price_from' => [
          'value' => $escape($submission_data['price_from'] ?? NULL),
        ],
        'field_paragraph_event_price_to' => [
          'value' => $escape($submission_data['price_to'] ?? NULL),
        ],
      ...
      foreach ($submission_data['requirements'] ?? [] as $requirement) {
        $event_details_data['field_event_requirements'][] = ['target_id' => $escape($requirement)];
      }
      ...
      $node = Node::create([
        'type' => 'event',
        'title' => $escape($submission_data['name_of_event'] ?? NULL),
        'field_event_details' => [
          'target_id' => $event_details->id(),
          'target_revision_id' => $event_details->getRevisionId(),
        ],
        'body' => [
          'value' => $escape($submission_data['description'] ?? NULL),
          'summary' => $truncator->truncateWords($escape($submission_data['description'] ?? NULL), 30, ''),
          'format' => 'rich_text',
        ],
        ...
        'field_node_link' => [
          'uri' => $escape($submission_data['website_url_for_event_information'] ?? NULL),
        ],
        'field_node_author' => [
          'value' => $escape($submission_data['contact_person'] ?? NULL),
        ],
        'field_node_email' => $escape($submission_data['contact_email_address'] ?? NULL),
        'field_node_phone' => $escape($submission_data['contact_telephone_number'] ?? NULL),
      ]);
      ...
      foreach ($submission_data['category'] ?? [] as $category) {
        $node->field_event_category->appendItem($escape($category));
      }
